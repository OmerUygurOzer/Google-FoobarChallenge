...
public class MinionInterrogation {
    ...
    public static int[] answer(int[][] minions) {
        ...
        int[] returner = new int[minions.length];
        for (int i = 0; i < minions.length; i++) {returner[i] = i;}


        //Go throught the minions like a bubble sort type of manner , use their weights(how efficient they will be at their spot) for comparisons
        for (int i = 0; i < minions.length - 1; i++) {
            for (int j = 1; j < minions.length - i; j++) {

                dec = (double) minions[j - 1][1] / minions[j - 1][2];
                timeLeft = minions[j - 1][0];
                timeRight = minions[j][0];


                double leftWeight = dec * timeLeft + (1 - (dec)) * (timeLeft + timeRight);

                dec = (double) minions[j][1] / minions[j][2];
                timeLeft = minions[j][0];
                timeRight = minions[j - 1][0];


                double rightWeight = dec * timeLeft + (1 - (dec)) * (timeLeft + timeRight);
                ...
                if (leftWeight > rightWeight) {
                    ...
                    tempInt = returner[j - 1];
                    returner[j - 1] = returner[j];
                    returner[j] = tempInt;
                }
