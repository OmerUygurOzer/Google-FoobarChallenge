...
public class ZombitInfection {
    public static int[][] answer(int[][] population, int x, int y, int strength) {
        ...
        /*
        Since we have a 2D array here , we can easily use a recursive method to solve this problem. We can simply modify the values of cells and still hold a instance to
        population in the initial call method.

        As we rabbits get infected, we spread the virus by calling the answer method on the cells nearby.
         */

        boolean willSpread = population[y][x] <= strength && population[y][x]!=-1;
        if(willSpread) {
            population[y][x] = -1;

            //Spread right
            if (x + 1 < population[0].length) {
